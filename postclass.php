...
class Post{
    ...
    public function insertpost($a_name, $a_content){

        $con     = $this->db->OpenCon();
        $title   = $con->real_escape_string($a_name);
        $content = $con->real_escape_string($a_content);
        //$img     = $con->real_escape_string($imgname);
        $query   = $con->prepare("INSERT INTO post(article_name, article_content) VALUES(?, ?)");
        $query->bind_param("sss", $title, $content);
    ...
    public function getarticle($articleid){
       ...
       // $stmt = "SELECT article_name,article_content,img,DATE_FORMAT(date,'%d %b, %Y') as dates from post WHERE article_id = '$id'";
        $stmt = "SELECT article_name,article_content,img,DATE_FORMAT(date,'%d %b, %Y') as dates from post";
